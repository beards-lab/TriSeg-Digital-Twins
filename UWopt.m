...
%
% load(sprintf('Sims_GA/P_NO%d.mat',PatID)); % if it does exist
% if MRI_flag == 1
%     load(sprintf('SimsUWwithCMR/P_NO%d.mat',PatID)); % load results from great lake clusters
% else
%     load(sprintf('SimsUWwithoutCMR0/P_NO%d.mat',PatID)); % load results from great lake clusters
% end
% m = output.m;
% m = [m m(end)];
% load(sprintf('SimsUWwithoutCMR0/P_NO%d.mat',PatID))
% m = output.modifiers;
m = 1*ones(1,length(mods)); % if the predefined modifiers do not exist
% m(9) = 10;
CurrentBestCost = evaluateModelUW(m,UWpatients,PatID); % call cost function in runSim.m
...
while true
[m, fval, exitflag, output] = patternsearch(@(m)evaluateModelUW(m,UWpatients,PatID), m, [], [], [], [],lb, ub, [], options_PS);
...
parfor i = 1:length(offsets)
    ...
    m_fmin = fminsearch(@(m_fmin)evaluateModelSubset(m_fmin,m,idx,UWpatients,PatID), m_fmin_initial, options_Fmin);
    optimized_m(:, i) = m_fmin;
    PotentialFmincosts(i) = evaluateModelSubset(m_fmin,m,idx,UWpatients,PatID);
end
...
output.mods = mods;
output.modifiers = m;
save(sprintf('SimsUWwithoutCMR0/P_NO%d.mat',PatID),"output");
